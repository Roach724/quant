...
def run_strategy(strategy_id: int, env: str) -> None:
    ...
    if lookback_bars > 0:
        ...
        # Restore scheduler state from file if available (persists across restarts).
        # If no state file exists, starts fresh with bar_count=0 (triggers lookback warmup).
        scheduler = RebalanceScheduler.from_file(
            file_path=str(scheduler_path),
            freq_minutes=freq_minutes,
            lookback_bars=lookback_bars,
            rebalance_every=int(live_cfg.get("rebalance_every", 1)),
        )
        if scheduler.bar_count > 0:
            log.info("Scheduler resumed: bar_count=%d last_rebalance=%s",
                     scheduler.bar_count, scheduler.last_rebalance_bar)
        else:
            log.info("Scheduler fresh start: freq=%dm lookback=%d bars rebalance_every=%d",
